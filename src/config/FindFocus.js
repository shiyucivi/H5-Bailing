(function(window) {
  if (
    !(
      typeof window["FindFocus"] === "object" &&
      window["FindFocus"] !== undefined
    )
  )
    window["FindFocus"] = {};
  ...
  function findFocusId(direction, list) {
    ...
    switch (direction) {
      case "left":
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (currentLoc.x >= loc.x + list[i].offsetWidth) {
            locele =
              Math.pow(
                Math.abs(currentLoc.x - loc.x - list[i].offsetWidth),
                2
              ) +
                Math.pow(Math.abs(currentLoc.y - loc.y), 2) >
              minDistance
                ? locele
                : list[i];
            minDistance =
      ...
      case "up":
        ...
        for (var i = 0; i < list.length; i++) {
          var loc = getPosition(list[i]);
          if (currentLoc.y >= loc.y + list[i].offsetHeight) {
            //当前元素一定在目标元素下面

            var d1 = Math.sqrt(
              Math.pow(Math.abs(currentLoc.x - loc.x), 2) +
                Math.pow(
                  Math.abs(currentLoc.y - loc.y - list[i].offsetHeight),
                  2
                )
            ); //左上角到左下角
            var d2 = Math.sqrt(
      ...
      case "right":
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (currentLoc.x + currentEle.offsetWidth <= loc.x) {
            locele =
              Math.pow(
                Math.abs(currentLoc.x + currentEle.offsetWidth - loc.x),
                2
              ) +
                Math.pow(Math.abs(currentLoc.y - loc.y), 2) >
              minDistance
        ...
        for (var i = 0; i < list.length; i++) {
          ...
          if (currentLoc.y + currentEle.offsetHeight <= loc.y) {
            locele =
              Math.pow(Math.abs(currentLoc.x - loc.x), 2) +
                Math.pow(
                  Math.abs(currentLoc.y + currentEle.offsetHeight - loc.y),
                  2
                ) >
              minDistance
                ? locele
